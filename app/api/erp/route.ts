...
export async function GET(request: Request) {
  try {
    ...
    const [directory, allTasks, allAttendance, tasks, attendance, activity] = await Promise.all([
      listUsers(), listAllTasks(), listAllAttendance(), listTasksFor(user), listAttendanceFor(user), listRecentActivity(),
    ]);
